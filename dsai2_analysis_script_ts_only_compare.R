...
model_sample_order <- 45 #x% of truth social
model_sample_p <- 2*4002115/(454459*(454459-1))
cutoff <- 0.106
model_pop_order <- round(model_sample_order/cutoff,0)

optimized_bfs <- function(graph, start_node, percentage) {
  total_nodes <- vcount(graph)
  cutoff <- ceiling(total_nodes * percentage)
  
  visited <- rep(FALSE, total_nodes)
  queue <- c(start_node)
  bfs_order <- c()
  
  visited[start_node] <- TRUE
  
  while (length(queue) > 0 && length(bfs_order) < cutoff) {
    node <- queue[1]
    queue <- queue[-1]
    bfs_order <- c(bfs_order, node)
...
generate_bfs_graph <- function(graph, percentage) {
  max_in_degree_node <- which.max(degree(graph, mode = "in"))
  bfs_order <- optimized_bfs(graph, max_in_degree_node, percentage)
  bfs_graph <- induced_subgraph(graph, bfs_order)
  
  return(bfs_graph)
}

main_gnp <- function(n, target_p, cutoff) {
  current_p <- 0.01
  error_margin <- 0.01
  max_iterations <- 10
  iteration <- 0
  ...
  while (iteration < max_iterations) {
    print("Create graph")
    g <- sample_gnp(n, current_p, directed = TRUE)
    print("BFS-search")
    bfs_g <- generate_bfs_graph(g, percentage = cutoff)
    estimated_p <- 2*ecount(bfs_g)/(vcount(bfs_g)*(vcount(bfs_g)-1))
    print("P-valuation")
    print(estimated_p)
    print(target_p)
    if (abs(estimated_p - target_p) / target_p <= error_margin) {
      cat("Target p value achieved within 1% error margin.\n")
      return(list(original_graph = g, bfs_graph = bfs_graph, estimated_p = estimated_p))
    }
    if (estimated_p > target_p) {
      current_p <- current_p / 10
    } else {
      current_p <- current_p * 10
    }
    print(current_p)
  ...
  cat("Maximum iterations reached without achieving target p value.\n")
  return(NULL)
}

res <- main_gnp(model_pop_order, model_sample_p, cutoff)
